...
let merge_work_directory_I _obj =
  ...
  let merge_bool = ref true in
  List.iter (fun (name, is_dir, h, obj) ->
    if not (Sys.file_exists name) then
      restore_work_directory obj
    else begin
      let local = read_text_object h in
      let remote =
        if is_dir then
          read_directory_object h
        else
          read_text_object h
      in
      if local <> remote then begin
        let write f content =
